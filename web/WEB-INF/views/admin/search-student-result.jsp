...
<div class="container theme-showcase" role="main">
    <div class="page-header">
        <h1 style="display: inline-block">查询到学生</h1>
        <div class="navbar-right col-sm-4" id="so" style="margin-top: 1.5em">
            ...
            <form class="searchForm" id="search-form" action="/admin/student/searchStudent" method="post">
                <input type="text" class="form-control" placeholder="学号查询" name="keyword" id="search-kw"/>
            </form>
    ...
    <div class="row">
        <div class="col-md-12">
            <table class="table">
                ...
                <tbody>
                <%
                    if (request.getAttribute("searchStudent")!=null) {

                %>
                    <tr>
                        <td>${searchStudent.student_id}</td>
                        <td>${searchStudent.name}</td>
                        <td>${searchStudent.password}</td>
                        <td>${searchStudent.sex}</td>
                        <td><fmt:formatDate value='${searchStudent.birthday}' pattern='yyyy-MM-dd '/></td>
                        <td>${searchStudent.phonenumber}</td>
                        <td>${searchStudent.address}</td>
                        <td><c:forEach items="${tutors}" var="tuto">
                            <c:if test="${tuto.tutor_id==searchStudent.tutor_id}">
                                ${tuto.tutor_name}
                            </c:if>
                        </c:forEach></td><td><c:forEach items="${classes}" var="clas">
                        <c:if test="${clas.class_id==searchStudent.class_id}">
                            ${clas.class_name}
                        </c:if>
                    </c:forEach></td>
                        <td><c:forEach items="${specialties}" var="speca">
                            <c:if test="${speca.specialty_id==searchStudent.specialty_id}">
                                ${speca.specialty_name}
                            </c:if>
                        </c:forEach></td>
                        <td><a data-toggle="modal" data-target="#${searchStudent.student_id}"><img src="/images/modify.png"></a><a
                                href="/admin/student/deleteStudent?student_id=${searchStudent.student_id}"><img
                                src="/images/delete.png"></a></td>
                        <div class="modal fade" id="${searchStudent.student_id}" tabindex="-1" role="dialog"
                                    ...
                                    <div class="modal-body">
                                        <form class="StudentInfoForm" role="form" action="/admin/student/updateStudent" method="post">
                                            <div class="form-group">
                                                <label>学生学号</label>
                                                <input type="number" name="update_stu_id" class="form-control"  readonly="readonly" value="${searchStudent.student_id}">
                                            </div>
                                            <div class="form-group">
                                                <label>学生姓名</label>
                                                <input type="text" name="new_stu_name" class="form-control" value="${searchStudent.name}" required>
                                            </div>
                                            <div class="form-group">
                                                <label>账户密码</label>
                                                <input type="text" name="new_stu_psw" class="form-control" value="${searchStudent.password}">
                                            </div>
                                            <div class="form-group">
                                                <label>性别</label>
                                                <input type="text" name="new_stu_sex" class="form-control" value="${searchStudent.sex}">
                                            </div>
                                            <div class="form-group">
                                                <label>生日</label>
                                                <input type="date"  name="new_stu_birthday" class="form-control" value=<fmt:formatDate value='${searchStudent.birthday}' pattern='yyyy-MM-dd '/>/>
                                            </div>
                                            <div class="form-group">
                                                <label>手机号</label>
                                                <input type="text" name="new_stu_phonenumber" class="form-control" value="${searchStudent.phonenumber}">
                                            </div><div class="form-group">
                                            <label>地址</label>
                                            <input type="text" name="new_stu_address" class="form-control" value="${searchStudent.address}">
                                        </div>
                                            ...
                                            <div class="form-group">
                                                ...
                                                <select id="new_stu_class" name="new_stu_class" class="form-control">
                                                    ...
                                                    <c:forEach items="${classes}" var="cla" varStatus="st">
                                                        <c:if test="${cla.class_id == searchStudent.class_id}">
                                            ...
                                            <div class="form-group">
                                                ...
                                                <select id="new_stu_tutor" name="new_stu_tutor" class="form-control">
                                                    ...
                                                    <c:forEach items="${tutors}" var="tur" varStatus="st">
                                                        <c:if test="${tur.tutor_id == searchStudent.tutor_id}">
                                            ...
                                            <div class="form-group">
                                                ...
                                                <select id="new_stu_specialty" name="new_stu_specialty" class="form-control">
                                                    ...
                                                    <c:forEach items="${specialties}" var="special" varStatus="st">
                                                        <c:if test="${special.specialty_id == searchStudent.specialty_id}">
